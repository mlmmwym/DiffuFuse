...
class Solver(object):
    def __init__(self, hyp_params, train_loader, dev_loader, test_loader, is_train=True, model=None, pretrained_emb=None):
        ...
        self.mosi_test_loader, self.mosei_test_loader , self.meld_test_loader, self.iemocap_test_loader = None, None , None, None
    ...
    def train_and_eval(self):
        wandb.login(key="3694d4706706a7771f959498249baf634133c779")
        wandb.init(project="UniMSE",name="1adapter", mode="offline")
        ...
        def train(model, optimizer):
            ...
            for i_batch, batch_data in enumerate(self.train_loader):
                # print("AAA")
                # print("{}/{}".format(i_batch+1,len(self.train_loader)))
                sentences, visual, vlens, acoustic, alens, y,labels_float, t5_input_id, t5_att_mask, t5_labels,_, _, ids,IS_BAG_list = batch_data
        ...
        def evaluate(model, loader, n_loader=None, test=False):
            ...
            with torch.no_grad():
                for i, batch in enumerate(loader):
                    ...
                    sentences, visual, vlens, acoustic, alens, y,labels_float, t5_input_id, t5_att_mask, t5_labels, _, _,ids,IS_BAG_list = batch
                    ids_list+=ids
                    # with torch.cuda.device(0):
                    #     if visual != None and acoustic != None:
                    #         visual, acoustic, t5_input_id, t5_att_mask, t5_labels = \
                    ...
                    logits, loss = self.model(sentences, t5_input_id, t5_att_mask,
                                          t5_labels, ids, visual, acoustic, vlens, alens,IS_BAG_list)
                    _,logits=torch.max(logits,dim=2)
                    # output_ids = self.model.generate(t5_input_id, t5_att_mask, visual, acoustic, vlens, alens)
                    # print(output_ids)
                    main_loss, tv_loss, ta_loss,diffusion_loss  = loss
                    # print(main_loss, tv_loss, ta_loss,diffusion_loss)
                # print('Training: tv_loss:{}, ta_loss:{}, main_loss:{}'.format(tv_loss, ta_loss, main_loss))
                    loss = main_loss + 0.0 * tv_loss + 0.0 * ta_loss+0.5*diffusion_loss
                    pred_token = self.tokenizer.batch_decode(logits, skip_special_tokens=True)
#                     if test:
#                         print('---------------------------------------------------------------------')
...
#                         print('truth token:{}'.format(y))
                    ...
                    if len(tmp) != len(list(labels_float)):
                        print('error')
                    total_loss += loss
                    results.extend(tmp)
                    truths.append(labels_float)
                    # ids_list.extend(ids)
        ...
        meld_acc_list=[]
        iemocap_acc_list=[]
        train_and_eval_info={'mosei_mae_list':[], 'mosei_corr_list':[], 'mosei_mult_a7_list':[], 'mosei_mult_a5_list':[],
                         'mosei_f_score_list':[], 'mosei_f_score_non0_list':[], 'mosei_acc_2_list':[], 'mosei_acc_2_non0_list':[],

                         'mosi_mae_list':[], 'mosi_corr_list':[], 'mosi_mult_a7_list':[], 'mosi_mult_a5_list':[],
                         'mosi_f_score_list':[], 'mosi_f_score_non0_list':[], 'mosi_acc_2_list':[], 'mosi_acc_2_non0_list':[],
                         
                         'train_loss_list':[],'val_loss_list':[],'mosei_test_loss_list':[],'mosi_test_loss_list':[]}
        mae_list, corr_list, mult_a7_list, mult_a5_list, f_score_list, f_score_non0_list, acc_2_list, acc_2_non0_list=[],[],[],[],[],[],[],[]
        ...
        for epoch in range(1, self.hp.num_epochs+1):
            ...
            train_and_eval_info['train_loss_list'].append(np.round(train_loss.detach().cpu().numpy(),4))
            train_and_eval_info['val_loss_list'].append(np.round(val_loss.detach().cpu().numpy(),4))
            ...
            if self.mosi_test_loader != None and self.mosei_test_loader != None and self.meld_test_loader != None and self.iemocap_test_loader != None:
                ...
                if self.hp.dataset == 'mosei_moseldmp':
                    args = get_args()
                    if epoch == 10:
                        args.visualize=True
                    else:
                    ...
                    print(mosi_results)
                    mosi_results = [ele.split(',')[1] if len(ele.split(',')) == 4 else '0.0' for ele in mosi_results]
                    # mosi_results = [ele.split(',')[1] for ele in mosi_results]
                    # print('mosi_results:{}'.format(mosi_results))
                    mosi_acc=eval_mosi(mosi_results, mosi_truths, mosi_ids_list, True)
                    ...
                    iemocap_results = [ele.split(',')[-1] if len(ele.split(',')) == 4 else 'neutral' for ele in iemocap_results]
                    # print(iemocap_results)
                    # iemocap_results = [ele.split(',')[-1] for ele in iemocap_results]
                    # print('iemocap_results:{}'.format(iemocap_results))

                    
                    iemocap_acc=eval_emotionlines(iemocap_results, iemocap_truths)
                    ...
                    mosi_acc_list.append(mosi_acc)
                    mosei_acc_list.append(mosei_acc)
                    meld_acc_list.append(meld_acc)
                    iemocap_acc_list.append(iemocap_acc)
                ...
                print(mosi_results)
                mae,corr,mult_a7,mult_a5,f_score,f_score_non0,acc_2,acc_2_non0=eval_mosei_senti(mosi_results, mosi_truths, mosi_ids_list, True)
                train_and_eval_info['mosi_mae_list'].append(mae)
                train_and_eval_info['mosi_corr_list'].append(corr)
                train_and_eval_info['mosi_mult_a7_list'].append(mult_a7)
                train_and_eval_info['mosi_mult_a5_list'].append(mult_a5)
                train_and_eval_info['mosi_f_score_list'].append(f_score)
                train_and_eval_info['mosi_f_score_non0_list'].append(f_score_non0)
                train_and_eval_info['mosi_acc_2_list'].append(acc_2)
                train_and_eval_info['mosi_acc_2_non0_list'].append(acc_2_non0)
                train_and_eval_info['mosi_test_loss_list'].append(np.round(mosi_test_loss.detach().cpu().numpy(),4))
                ...
                wandb.log(
                    ...
                    "mosi_mae_list": mae,
                    "mosi_corr_list": corr,
                    "mosi_mult_a7_list": mult_a7,
                    "mosi_mult_a5_list": mult_a5,
                    "mosi_f_score_list": f_score,
                    "mosi_f_score_non0_list": f_score_non0,
                    "mosi_acc_2_list": acc_2,
                    "mosi_acc_2_non0_list": acc_2_non0,
                    "mosi_test_loss_list": np.round(mosi_test_loss.detach().cpu().numpy(),4),
                        }
                    )
                )
                ...
                mae,corr,mult_a7,mult_a5,f_score,f_score_non0,acc_2,acc_2_non0=eval_mosei_senti(mosei_results, mosei_truths, mosei_ids_list, True)
                train_and_eval_info['mosei_mae_list'].append(mae)
                train_and_eval_info['mosei_corr_list'].append(corr)
                train_and_eval_info['mosei_mult_a7_list'].append(mult_a7)
                train_and_eval_info['mosei_mult_a5_list'].append(mult_a5)
                train_and_eval_info['mosei_f_score_list'].append(f_score)
                train_and_eval_info['mosei_f_score_non0_list'].append(f_score_non0)
                train_and_eval_info['mosei_acc_2_list'].append(acc_2)
                train_and_eval_info['mosei_acc_2_non0_list'].append(acc_2_non0)
                train_and_eval_info['mosei_test_loss_list'].append(np.round(mosei_test_loss.detach().cpu().numpy(),4))
                ...
                wandb.log(
                    (
                        {
                    "mosei_mae_list": mae,
                    "mosei_corr_list": corr,
                    "mosei_mult_a7_list": mult_a7,
                    "mosei_mult_a5_list": mult_a5,
                    "mosei_f_score_list": f_score,
                    "mosei_f_score_non0_list": f_score_non0,
                    "mosei_acc_2_list": acc_2,
                    "mosei_acc_2_non0_list": acc_2_non0,
                    "mosei_test_loss_list": np.round(mosei_test_loss.detach().cpu().numpy(),4),
                    "train_loss":train_loss,
                    "val_loss":val_loss,
                    "mosi_test_loss":mosi_test_loss,
                    "mosei_test_loss":mosei_test_loss
                        }
                    )
                )
                # if val_loss < best_valid:
                ...
                mae,corr,mult_a7,mult_a5,f_score,f_score_non0,acc_2,acc_2_non0=eval_mosei_senti(results, truths, ids_list, True)
                mae_list.append(mae)
                corr_list.append(corr)
                mult_a7_list.append(mult_a7)
                mult_a5_list.append(mult_a5)
                f_score_list.append(f_score)
                f_score_non0_list.append(f_score_non0)
                acc_2_list.append(acc_2)
                acc_2_non0_list.append(acc_2_non0)

                train_loss_list.append(np.round(train_loss.detach().cpu().numpy(),4))
                test_loss_list.append(np.round(test_loss.detach().cpu().numpy(),4))
                val_loss_list.append(np.round(val_loss.detach().cpu().numpy(),4))

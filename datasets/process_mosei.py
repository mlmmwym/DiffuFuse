...
pickle_filename = './MOS/mosei_data_0610.pkl'
csv_filename = './new_MOSEI-label-v4.csv'
...
all_csv_id = [vid[i]+'_'+str(cid[i]) for i in range(len(vid))]

##做vid 和 cid之间的映射关系
for i, idd in enumerate(all_csv_id):
...
to_pickle(train, './mosei_train.pkl')
to_pickle(dev, './mosei_dev.pkl')
to_pickle(test, './mosei_test.pkl')